...
class QuantizedBartEncoderLayer(QuantizedModule):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.self_attn_layer_norm = org_module.self_attn_layer_norm
        self.dropout = org_module.dropout
        ...
        self.final_layer_norm = org_module.final_layer_norm
        ...
        if qoutput:
            self.encoder_act_fn_post_act_fake_quantize = Quantizer(None, a_qconfig)
    ...
    def forward(
        ...
        if self.qoutput:
            hidden_states = self.encoder_act_fn_post_act_fake_quantize(hidden_states, observation_mask, 1)
...
class QuantizedBartDecoderLayer(QuantizedModule):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.encoder_attn_layer_norm = org_module.encoder_attn_layer_norm
        self.fc1 = Quantizer(org_module.fc1, w_qconfig)
        self.fc2 = Quantizer(org_module.fc2, w_qconfig)
        self.final_layer_norm = org_module.final_layer_norm

        self.self_attn_layer_norm_post_act_fake_quantize = Quantizer(None, a_qconfig)
        self.encoder_attn_layer_norm_post_act_fake_quantize = Quantizer(None, a_qconfig)
        self.fc1_post_act_fake_quantize = Quantizer(None, a_qconfig)
        if self.backend == 'tensorrt':
        ...
        if self.qoutput:
            self.final_layer_norm_post_act_fake_quantize = Quantizer(None, a_qconfig)
    ...
    def forward(
        ...
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states)
        hidden_states = self.self_attn_layer_norm_post_act_fake_quantize(hidden_states, observation_mask, 1)
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states = self.encoder_attn_layer_norm_post_act_fake_quantize(hidden_states, observation_mask , 1)
        ...
        if self.qoutput:
            hidden_states = self.final_layer_norm_post_act_fake_quantize(hidden_states, observation_mask, 1)
...
class QuantizedBartEncoder(QuantizedModule, ModuleUtilsMixin):
    ...
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.layernorm_embedding = org_module.layernorm_embedding

        self.gradient_checkpointing = org_module.gradient_checkpointing

        self.layernorm_embedding_post_act_fake_quantize = Quantizer(None, a_qconfig)
    ...
    def forward(
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = self.layernorm_embedding_post_act_fake_quantize(hidden_states, observation_mask, 1)
...
class QuantizedBartModel(QuantizedModule, ModuleUtilsMixin):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.encoder = QuantizedBartEncoder(org_module.encoder, w_qconfig, a_qconfig, qoutput=True, backend=self.backend)
        self.decoder = QuantizedBartDecoder(org_module.decoder, w_qconfig, a_qconfig, qoutput=self.qoutput, backend=self.backend)
