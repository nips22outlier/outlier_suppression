...
class QuantizedRobertaEmbeddings(QuantizedModule):
    ...
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.LayerNorm = org_module.LayerNorm
    ...
    def forward(
        ...
        if self.position_embedding_type == "absolute":
            logger.info('the device of position_ids is {}'.format(position_ids))
            position_embeddings = self.position_embeddings(position_ids)
            embeddings += position_embeddings
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        if self.qoutput:
            embeddings = self.dropout_post_act_fake_quantize(embeddings, observation_mask, 1)
        return embeddings
