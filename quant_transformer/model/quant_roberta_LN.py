"""PyTorch Quantized Roberta model. """
# TODO: relative keys
...
class QuantizedRobertaLayerNorm(QuantizedModule):
...
class QuantizedRobertaLayerNormSplit(QuantizedModule):
...
class QuantizedRobertaEmbeddings(QuantizedModule):
    ...
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.LayerNorm = QuantizedRobertaLayerNormSplit(
            org_module.LayerNorm, w_qconfig, a_qconfig,
...
class QuantizedRobertaSelfOutput(QuantizedModule):
    ...
    def forward(self, hidden_states, input_tensor, observation_mask=None):
        ...
        input_tensor = input_tensor * self.delay_ln
        if self.backend == 'tensorrt':
            hidden_states = self.dropout_post_act_fake_quantize(hidden_states, observation_mask, 1)
        hidden_states = self.LayerNorm(hidden_states + input_tensor, observation_mask)
        return hidden_states
...
class QuantizedRobertaEncoder(QuantizedModule):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.layer[self.config.num_hidden_layers - 1].output.LayerNorm \
            = QuantizedRobertaLayerNorm(
                org_module.layer[self.config.num_hidden_layers - 1].output.LayerNorm,
                w_qconfig, a_qconfig, qoutput=self.qoutput, backend=self.backend)
...
class QuantizedRobertaPooler(QuantizedModule):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        if qoutput:
            self.activation_post_act_fake_quantize = Quantizer(None, a_qconfig)
    ...
    def forward(self, hidden_states):
        ...
        if self.qoutput:
            pooled_output = self.activation_post_act_fake_quantize(pooled_output)
...
class QuantizedRobertaForSequenceClassification(QuantizedModule):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic', is_remove_padding=False):
        ...
        self.config = org_module.config
        self.qoutput = qoutput
        self.roberta = QuantizedRobertaModel(org_module=org_module.roberta, w_qconfig=w_qconfig, a_qconfig=a_qconfig, qoutput=False, backend=self.backend)
        self.classifier = QuantizedRobertaClassificationHead(org_module.classifier, w_qconfig=w_qconfig, a_qconfig=a_qconfig, qoutput=self.qoutput, backend=self.backend)
