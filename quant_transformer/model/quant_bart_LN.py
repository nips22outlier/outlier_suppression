"""PyTorch Quantized BARTLN model. """
import copy
import math
import random
import warnings
...
class QuantizedBartLayerNorm(QuantizedModule):
...
class QuantizedBartLayerNormSplit(QuantizedModule):

    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        super().__init__(backend=backend)
        self.qoutput = qoutput
        self.norm = nn.LayerNorm(org_module.normalized_shape, elementwise_affine=False)
        tmp = (org_module.bias.data / org_module.weight.data).detach().clone()
        self.bias = torch.nn.Parameter(tmp)
        if self.qoutput:
            self.norm_post_act_fake_quantize = Quantizer(None, a_qconfig)

    def forward(self, hidden_states, observation_mask=None):
        hidden_states = self.norm(hidden_states)
        hidden_states += self.bias
        if self.qoutput:
            hidden_states = self.norm_post_act_fake_quantize(hidden_states, observation_mask, 1)
        return hidden_states
...
class QuantizedBartAttention(QuantizedModule):
    ...
    def __init__(
        ...
        self.att_post_act_fake_quantize = Quantizer(None, a_qconfig)
        if self.qoutput:
    ...
    def forward(
        ...
        # get query proj
        query_states = self.query_permute_post_act_fake_quantize(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_states = self._shape(
                self.key_transpose_post_act_fake_quantize(self.k_proj(key_value_states), observation_mask, 1),
                -1,
                bsz
            )
            value_states = self._shape(
                self.value_permute_post_act_fake_quantize(self.v_proj(key_value_states), observation_mask, 1),
                -1,
                bsz
            )
            ...
            key_states = self._shape(
                self.key_transpose_post_act_fake_quantize(self.k_proj(hidden_states), observation_mask, 1),
                -1,
                bsz
            )
            value_states = self._shape(
                self.value_permute_post_act_fake_quantize(self.v_proj(hidden_states), observation_mask, 1),
                -1,
                bsz
            )
        ...
        attn_output = self.att_post_act_fake_quantize(attn_output, observation_mask, 1)
...
class QuantizedBartEncoderLayer(QuantizedModule):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.qoutput = qoutput
        self.embed_dim = org_module.embed_dim
        self.self_attn = QuantizedBartAttention(org_module.self_attn, w_qconfig, a_qconfig, qoutput=False, backend=self.backend)
        self.self_attn_layer_norm = QuantizedBartLayerNormSplit(
            org_module.self_attn_layer_norm,
            w_qconfig,
            a_qconfig,
            qoutput=True,
            backend=backend
        )
        self.delay_ln_self_attn = None
        ...
        self.final_layer_norm = QuantizedBartLayerNormSplit(
            org_module.final_layer_norm,
            w_qconfig,
            a_qconfig,
            qoutput=self.qoutput,
            backend=backend
        )
        self.delay_ln_final = None

        self.fc1_post_act_fake_quantize = Quantizer(None, a_qconfig)
    ...
    def forward(
        ...
        residual = residual * self.delay_ln_self_attn
        ...
        if self.backend == 'tensorrt':
            hidden_states = self.dropout_post_act_fake_quantize(hidden_states, observation_mask, 1)
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states, observation_mask)

        # ffn residual
        residual = hidden_states
        residual = residual * self.delay_ln_final
        hidden_states = self.activation_fn(self.fc1(hidden_states))
        hidden_states = nn.functional.dropout(hidden_states, p=self.activation_dropout, training=self.training)
        hidden_states = self.fc1_post_act_fake_quantize(hidden_states, observation_mask, 1)
...
class QuantizedBartDecoderLayer(QuantizedModule):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.activation_dropout_post_act_fake_quantize = Quantizer(None, a_qconfig)
        ...
        self.encoder_attn = QuantizedBartAttention(org_module.encoder_attn, w_qconfig, a_qconfig, qoutput=False, backend=self.backend)
        self.encoder_attn_layer_norm = QuantizedBartLayerNormSplit(
            org_module.encoder_attn_layer_norm,
            w_qconfig,
            a_qconfig,
            qoutput=True,
            backend=backend
        )
        self.delay_ln_encoder_attn = None
        ...
        self.fc1_post_act_fake_quantize = Quantizer(None, a_qconfig)
        if self.backend == 'tensorrt':
            self.dropout_self_attn_post_act_fake_quantize = Quantizer(None, a_qconfig)
            self.dropout_encoder_attn_post_act_fake_quantize = Quantizer(None, a_qconfig)
            self.fc2_post_act_fake_quantize = Quantizer(None, a_qconfig)
    ...
    def forward(
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        if self.backend == 'tensorrt':
            hidden_states = self.dropout_self_attn_post_act_fake_quantize(hidden_states, observation_mask, 1)
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states, observation_mask)
        ...
        if encoder_hidden_states is not None:
            residual = hidden_states
            residual = residual * self.delay_ln_encoder_attn
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            if self.backend == 'tensorrt':
                hidden_states = self.dropout_encoder_attn_post_act_fake_quantize(hidden_states, observation_mask, 1)
            hidden_states = residual + hidden_states
...
class QuantizedBartEncoder(QuantizedModule, ModuleUtilsMixin):
    """
    ...
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        self.embed_positions = QuantizedBartLearnedPositionalEmbedding(org_module.embed_positions, w_qconfig, a_qconfig, qoutput=True, backend=self.backend)

        self.layers = nn.ModuleList()
        for i in range(self.config.encoder_layers):
            if i != self.config.encoder_layers - 1:
                self.layers.append(QuantizedBartEncoderLayer(org_module.layers[i], w_qconfig, a_qconfig, qoutput=True, backend=self.backend))
            else:
                self.layers.append(QuantizedBartEncoderLayer(org_module.layers[i], w_qconfig, a_qconfig, qoutput=self.qoutput, backend=self.backend))
        
        # because there is no short-cut here, recover it to normal layernorm
        self.layers[self.config.encoder_layers - 1].final_layer_norm = QuantizedBartLayerNorm(
            org_module.layers[self.config.encoder_layers - 1].final_layer_norm,
            w_qconfig,
            a_qconfig,
            qoutput=self.qoutput,
            backend=backend
        ...
        self.layernorm_embedding = QuantizedBartLayerNormSplit(
            org_module.layernorm_embedding,
            w_qconfig,
            a_qconfig,
            qoutput=True,
            backend=backend
    ...
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        for i in range(self.config.decoder_layers):
            if i != self.config.decoder_layers - 1:
                self.layers.append(QuantizedBartDecoderLayer(org_module.layers[i], w_qconfig, a_qconfig, qoutput=True, backend=self.backend))
            else:
                self.layers.append(QuantizedBartDecoderLayer(org_module.layers[i], w_qconfig, a_qconfig, qoutput=self.qoutput, backend=self.backend))
        
        self.layers[self.config.decoder_layers - 1].final_layer_norm = QuantizedBartLayerNorm(
            org_module.layers[self.config.decoder_layers - 1].final_layer_norm,
            w_qconfig,
            a_qconfig,
            qoutput=self.qoutput,
            backend=backend
...
class QuantizedBartModel(QuantizedModule, ModuleUtilsMixin):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        super().__init__(backend=backend)
        self.qoutput = qoutput
        self.config = org_module.config
        ...
        self.encoder = QuantizedBartEncoder(
            org_module.encoder,
            w_qconfig,
            a_qconfig,
            qoutput=True,
        ...
        self.decoder = QuantizedBartDecoder(
            org_module.decoder,
            w_qconfig,
            a_qconfig,
            qoutput=self.qoutput,
...
class QuantizedBartForConditionalGeneration(QuantizedModule, GenerationMixin, ModuleUtilsMixin):
