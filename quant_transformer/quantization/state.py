...
def enable_cosine_function(model, quantizer_type='fake_quant', except_quantizer=None):
    logger.info('Enable cosine similarity between the quantized value and the real one')
    for name, submodule in model.named_modules():
        if isinstance(submodule, QuantizeBase):
            if (quantizer_type not in name) or \
               (except_quantizer is not None and name in except_quantizer):
                logger.debug('The except_quantizer is {}'.format(name))
                submodule.cosine = False
                continue
            logger.debug('Enable cosine: {}'.format(name))
            submodule.set_name(name)
            submodule.cosine = True
